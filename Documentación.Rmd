...
save(ajuste, file='ajuste covid.RData')
```
...
Para realizar predicciones a horizonte $h$ a partir de un modelo de regresión es necesario tener predicciones de los valores de las variables regresoras a ese mismo horizonte. Estos valores se pueden obtener a partir de:

- En caso de que alguna variable haya sido introducida con cierto retardo, utilizar estos valores "sobrantes".
- En caso de que el parámetro `max_lag` (que recorta la serie)
...
```{r}
# Recortamos las variables
max_lag <- 1
close <- window(close, start=start(close)[1]+max_lag)
open <- window(open, start=start(open)[1]+max_lag)
high <- window(high, start=start(high)[1]+max_lag)
low <- window(low, start=start(low)[1]+max_lag)
volume <- window(volume, start=start(volume)[1]+max_lag)
```



**Fase 1**: Una vez recortadas las series, se aplica de nuevo el preblanqueado para cada par $(X_i, Y)$ para eliminar la correlación espuria y obtener el retardo significativo (esta vez el retardo $k$ debe ser menor o igual a 0).

```{r}
alpha <- 0.05      # nivel de significación
trend <- function(x) {return(suppressWarnings(adf.test(x)$p.value) >= alpha)}


# Preblanqueado para la variable open
open_diff <- open; close_diff <- close
while (any(apply(cbind(open_diff, close_diff), 2, trend))) {
    open_diff <- diff(open_diff)
...
high_diff <- high; close_diff <- close
while (any(apply(cbind(high_diff, close_diff), 2, trend))) {
...
# Preblanqueado con la variable low
low_diff <- low; close_diff <- close
while (any(apply(cbind(low_diff, close_diff), 2, trend))) {
...
# Preblanqueado con la variable volume
volume_diff <- volume; close_diff <- close
while (any(apply(cbind(volume_diff, close_diff), 2, trend))) {
...
Como podemos apreciar, todas las variables regresoras tienen su correlación máxima en el retardo $k=0$, por lo que no es necesario alterar ninguna de las series para construir un modelo para cada par $(X_i,Y)$. 

A continuación, procedemos a ajustar un modelo ARIMA (sin la restricción de estacionariedad sobre sus residuos) para cada variable candidata a regresora. Podemos ajustar este modelo a mano (utilizando la función `stats::auto.arima` y retirando los coeficientes no significativos), pero para este caso utilizaremos la función ya descrita anteriormente, `auto.fit.arima`, que realiza todo este proceso de forma automática.

*Nota*: En este caso utilizaremos como cirterio de información el BIC.
...
ajuste_high <- auto.fit.arima(close, xregs=cbind(high=high), show_info=F)
ajuste_high
...
```{r}
data_high <- cbind(close, open, high=window(high_lagged, start=start(close)))
head(data_high)

data_low <- cbind(close, open, low=window(low_lagged, start=start(close)))
head(data_low)

data_volume <- cbind(close, open, volume)
head(data_volume)
```
...
- El mejor modelo para la variable `High` y `Open` es un ARIMA(0,0,0) con AICc=5918.6 y retardo $k=-1$ (se ha mejorado el modelo anterior).
- El mejor modelo para la variable `Low` y `Open` es un ARIMA(0,0,0) con AICc=5921.36 y retardo $k=-1$ (se ha mejorado el modelo anterior).
...
```{r}
response <- residuals(ajuste_low, type='regression')
head(response)
```
...
```{r}
# función modificada para ignorar valores nulos
trend <- function(x) {return(suppressWarnings(adf.test(x[!is.na(x)])$p.value) > alpha)}
...
```{r}
data_low <- cbind(close, open, high=window(high_lagged, start=start(close)), 
                  low=window(low_lagged, start=start(close)))
head(data_low)

data_volume <- cbind(close, open, high=window(high_lagged, start=start(close)), volume)
head(data_volume)
```
...
- El mejor modelo para la variable `Low`, `High`, `Open` es un ARIMA(0,0,0) con AICc=5920.59 y retardo $k=-1$ (no se ha mejorado el modelo anterior).
...
regresoras <- as.data.frame(
    lapply(microsoft[, c('Open', 'High', 'Low', 'Volume')], ts))
auto.fit.arima.regression(serie=close, xregs=regresoras, ic='aicc', stationary_method='adf.test')
```
