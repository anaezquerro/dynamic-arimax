...
Tiempo de ejecución de la función `auto.fit.arima.regression` secuencial:

```{r}
print(elapsed_time, row.names=F)
```

Se proporciona una versión paralelizada de `auto.fit.arima.regression` que realiza el proceso de ajustar modelos con las variables regresoras del conjunto dado de forma paralela (usando todos los procesadores disponibles de la máquina):
...
regresoras <- as.data.frame(lapply(microsoft[, c('Open', 'High', 'Low', 'Volume')], ts))
