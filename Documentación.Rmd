...
```{r, eval=F}
forecast_model(ajuste, h, mode='bootstrap', levels=c(80, 90))
```
...
-   `mode` [`character`]: Modo de realizar las predicciones: basadas en normalidad sobre los residuos (`norm`) o a través de *bootstrap* (`bootstrap`). Por defecto se realizan a través de bootstrap.
...
preds <- forecast_model(Y, data.frame(X=X), ajuste_patatas, h=10, mode='bootstrap')
