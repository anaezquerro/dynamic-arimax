...
% beamer style 
\definecolor{journalred}{HTML}{c8484d}
\definecolor{jorunalblue}{HTML}{3e64b9}
\definecolor{journalgrey}{HTML}{888586}
\hypersetup{
    colorlinks = true,
    linkcolor = jorunalblue,
    urlcolor = jorunalblue
}


\setbeamercolor{palette primary}{bg=journalred,fg=white}
\setbeamercolor{palette secondary}{bg=jorunalblue,fg=white}
\setbeamercolor{palette tertiary}{bg=journalgrey, fg=white}
\setbeamercolor{palette quaternary}{bg=journalred, fg=white}
\setbeamercolor{structure}{fg=journalred} % itemize, enumerate, etc
\setbeamercolor{section in toc}{fg=jorunalblue} % TOC sections
\setbeamercolor{subsection in head/foot}{bg=jorunalblue,fg=white}
...
\titlegraphic{\vspace{0.3em}
\includegraphics[scale=0.25]{gallery/modes.png}
\hspace{1em}
\includegraphics[scale=0.25]{gallery/usc.png}
\hspace{1em}
\includegraphics[scale=0.2]{gallery/logo.pdf}
}
...
\begin{frame}{Introduction}
        The \textbf{linear dynamic regression model (DRM)} defines the linear dependence between a stochastic process $Y_t$ and a set of processes $\mathcal{X} =  \{ X_t^{(1)}, ..., X_t^{(m)} \}$:
...
\begin{frame}{Methodology}
    ...
    \begin{enumerate}
        \item Initialization. Consider $X_t^\text{best}$ as the covariate (lagged $r$ moments) obtained via \textit{prewhitening} that minimizes the IC of the constructed model with $Y_t$:
...
\begin{frame}{Simulation procedure}
    \begin{enumerate}
        \item Artificially construct $Y_t$:
        \[ Y_t = \beta_0 + \beta_1 X_{t-r_1}^{(1)} + \beta_2 X_{t-r_2}^{(2)} + \beta X_{t-r_3}^{(3)} + \eta_t \] 
        where $\eta_t\sim\text{ARMA}(p,q)$, $\beta_0,...,\beta_3$, $r_i\in[0,6]$ and $X_t^{(1)}$, $X_t^{(2)}$ and $X_t^{(3)}$ are randomly generated.
        \item Consider the set of candidates:
        \[ \mathcal{X} = \{ X_t^{(1)}, X_t^{(2)}, X_t^{(3)}, \underbrace{X_t^{(4)}, X_t^{(5)}, X_t^{(6)}}
        _{\text{not in the model}}\}  \]
...
\section{Application on real cases}
