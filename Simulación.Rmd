...
Y <- beta0 + beta1 * X1$X + beta2 * X2$X + beta3 * X3$X + 0.1*residuals$X
```


Ajustamos el modelo con las variables originales (no diferenciamos ninguna):
```{r}
regresoras <- data.frame(X1=X1$X, X2=X2$X, X3=X3$X, X4=X4$X, X5=X5$X, X6=X6$X) auto.fit.arima.regression(Y, regresoras, seasonal=F, show_info=T) 
```


- Podemos observar cómo la función
...
beta0 <- 0.8; beta1 <- -1.8; beta2 <- 7.12; beta3 <- 12.3
r1 <- -2; r3 <- -1
Y <- beta0 + beta1 * lag(X1$X, r1) + beta2 * X2$X + beta3 * lag(X3$X, r3) + 1.5*residuals$X

regresoras <- data.frame(X1=X1$X, X2=X2$X, X3=X3$X, X4=X4$X, X5=X5$X, X6=X6$X)
auto.fit.arima.regression(Y, regresoras, show_info=T, stationary_method='adf.test')
```
